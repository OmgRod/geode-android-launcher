...
inline bool set_mprotect(std::uintptr_t address, size_t size, int protection) {
...
inline constexpr std::uint32_t elfhash(std::string_view name) {
    std::uint32_t h = 0, g;
...
inline constexpr std::array<const std::uint8_t, 3> SYMBOL_PATCH_VALUE{":3"};

inline bool patch_symbol(std::uint32_t* hash_table, char* str_table, ElfW(Sym)* sym_table, const std::string_view orig_name) {
...
inline bool perform_symbol_patches(dl_phdr_info* info, const ElfDynamicState& state) {
    // patch symbol names
...
inline bool rewrite_symbol_reloc(std::uintptr_t addr, T& r, ElfW(Sym)* sym, void* func) {
...
template <typename T> requires is_one_of<T, ElfW(Rela), ElfW(Rel)>
inline bool perform_reloc_patches(dl_phdr_info* info, const ElfDynamicState& state) {
...
inline int on_dl_iterate(dl_phdr_info* info, size_t size, void* data) {
